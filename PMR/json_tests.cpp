...
void bench(string_view verb, file_list const &vf, impl_list const &vi,
           std::size_t Trials) {
    ...
    for (unsigned j = 0; j < vi.size(); ++j) {
      ...
      for (unsigned k = 0; k < Trials; ++k) {
        ...
        result.mbs = static_cast<std::size_t>(
            (0.5 + 1000.0 * static_cast<double>(result.calls) * static_cast<double>(vf[i].text.size()) / static_cast<double>(result.millis) /
                       1024 / 1024));
      ...
      auto const calls = std::accumulate(
          trial.begin(), trial.end(), std::size_t{},
          [](std::size_t lhs, sample const &rhs) { return lhs + rhs.calls; });
      auto const millis = std::accumulate(
          trial.begin(), trial.end(), std::size_t{},
          [](std::size_t lhs, sample const &rhs) { return lhs + rhs.millis; });
...
static bool parse_option(char const *s) {
...

  {
    ...
    if (k > 0) {
      s_trials = static_cast<std::size_t>(k);
    } else {
      return false;
    }
  }
...
static bool add_impl(impl_list &vi, char impl) {
  ...
  case 'd':
    ...
    vi.emplace_back(new boost_pmr_impl(s_branch));
    break;
