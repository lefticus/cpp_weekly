...
static void Monotonic_Vector_Heap_Create_Destroy(benchmark::State &state)
{
  // Remember that free with monotonic buffer is a noop, so destruction is much faster
  for (auto _ : state) {
    std::pmr::monotonic_buffer_resource mem_resource(sizeof(std::pmr::string) * 1000);
...
BENCHMARK(Monotonic_Vector_Heap_Create_Destroy);
...
static void Monotonic_Set_Heap_Create_Destroy(benchmark::State &state)
{
  for (auto _ : state) {
    std::pmr::monotonic_buffer_resource mem_resource(sizeof(std::pmr::string) * 1000);
    std::pmr::polymorphic_allocator alloc{ &mem_resource };

    [[maybe_unused]] auto obj = std::pmr::set<std::pmr::string>{ std::begin(long_strings), std::end(long_strings), &mem_resource };
    benchmark::DoNotOptimize(obj);
  }
}
BENCHMARK(Monotonic_Set_Heap_Create_Destroy);

static void Monotonic_Set_StringView_Heap_Create_Destroy(benchmark::State &state)
{
  for (auto _ : state) {
    std::pmr::monotonic_buffer_resource mem_resource(sizeof(std::pmr::string) * 1000);
    std::pmr::polymorphic_allocator alloc{ &mem_resource };

    [[maybe_unused]] auto obj = std::pmr::set<std::string_view>{ std::begin(long_string_views), std::end(long_string_views), &mem_resource };
    benchmark::DoNotOptimize(obj);
  }
}
BENCHMARK(Monotonic_Set_StringView_Heap_Create_Destroy);
...
static void Std_Set_StringView_Create_Destroy(benchmark::State &state)
{
  for (auto _ : state) {
    std::set<std::string_view> words{ std::begin(long_string_views), std::end(long_string_views) };
    benchmark::DoNotOptimize(words);
  }
}
BENCHMARK(Std_Set_StringView_Create_Destroy);


static void Monotonic_Set_Heap_Create_Destroy_Pass_Alloc_Object(benchmark::State &state)
{
  for (auto _ : state) {
    std::pmr::monotonic_buffer_resource mem_resource(sizeof(std::pmr::string) * 1000);
    std::pmr::polymorphic_allocator alloc{ &mem_resource };

    [[maybe_unused]] auto obj = std::pmr::set<std::pmr::string>{ std::begin(long_strings), std::end(long_strings), alloc };
    benchmark::DoNotOptimize(obj);
  }
}
BENCHMARK(Monotonic_Set_Heap_Create_Destroy_Pass_Alloc_Object);


static void Monotonic_Set_Heap_Create_Blink_Out(benchmark::State &state)
{
  for (auto _ : state) {
    std::pmr::monotonic_buffer_resource mem_resource(sizeof(std::pmr::string) * 1000);
    std::pmr::polymorphic_allocator alloc{ &mem_resource };

    [[maybe_unused]] auto *obj = alloc.new_object<std::pmr::set<std::pmr::string>>(std::begin(long_strings), std::end(long_strings));
    benchmark::DoNotOptimize(*obj);
  }
}
BENCHMARK(Monotonic_Set_Heap_Create_Blink_Out);

static void Std_Set_Create_Destroy(benchmark::State &state)
{
  for (auto _ : state) {
    std::set<std::string> words{ std::begin(long_strings), std::end(long_strings) };
    benchmark::DoNotOptimize(words);
  }
}
BENCHMARK(Std_Set_Create_Destroy);
...
static void Std_Vector_Create_Sort_Destroy(benchmark::State &state)
{
  for (auto _ : state) {
    std::vector<std::string> words{ std::begin(long_strings), std::end(long_strings) };
    std::sort(std::begin(words), std::end(words));
    benchmark::DoNotOptimize(words);
  }
}
BENCHMARK(Std_Vector_Create_Sort_Destroy);

static void Monotonic_Vector_Create_Sort_Destroy(benchmark::State &state)
{
  for (auto _ : state) {
    std::pmr::monotonic_buffer_resource mem_resource(sizeof(std::pmr::string) * 1000);
    std::pmr::polymorphic_allocator alloc{ &mem_resource };

    std::pmr::vector<std::pmr::string> words{ std::begin(long_strings), std::end(long_strings), &mem_resource };
    std::sort(std::begin(words), std::end(words));
    benchmark::DoNotOptimize(words);
  }
}
BENCHMARK(Monotonic_Vector_Create_Sort_Destroy);


static void Monotonic_Vector_Heap_Create_Blink_Out(benchmark::State &state)
{
  for (auto _ : state) {
    std::pmr::monotonic_buffer_resource mem_resource(sizeof(std::pmr::string) * 1000);
    std::pmr::polymorphic_allocator alloc{ &mem_resource };

    [[maybe_unused]] auto *obj = alloc.new_object<std::pmr::vector<std::pmr::string>>(std::begin(long_strings), std::end(long_strings));
    benchmark::DoNotOptimize(*obj);
  }
}
BENCHMARK(Monotonic_Vector_Heap_Create_Blink_Out);

static void Monotonic_Vector_Heap_Copy_Blink_Out(benchmark::State &state)
{
  const auto orig = std::pmr::vector<std::pmr::string>(std::begin(long_strings), std::end(long_strings));

  for (auto _ : state) {
    std::pmr::monotonic_buffer_resource mem_resource(sizeof(std::pmr::string) * 1000);
    std::pmr::polymorphic_allocator alloc{ &mem_resource };

    [[maybe_unused]] auto *obj = alloc.new_object<std::pmr::vector<std::pmr::string>>(orig);
    benchmark::DoNotOptimize(*obj);
  }
}
BENCHMARK(Monotonic_Vector_Heap_Copy_Blink_Out);


static void Std_Vector_Create_Destroy(benchmark::State &state)
{
  for (auto _ : state) {
    [[maybe_unused]] auto words = std::vector<std::string>(std::begin(long_strings), std::end(long_strings));
    benchmark::DoNotOptimize(words);
  }
}
BENCHMARK(Std_Vector_Create_Destroy);
