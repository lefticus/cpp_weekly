...
struct PCG
{
  struct pcg32_random_t { std::uint64_t state=0;  std::uint64_t inc=7; };
  pcg32_random_t rng;
...
// Fit with default -ftemplate-depth=900 (884+2)
// 11Gb of RAM & 12min of compilation on a Intel(R) Core(TM) i7-9850H CPU @ 2.60GHz (mono thread compilation...)
constexpr int width = 34; // 1024
constexpr int height = 26; // 768
constexpr int samps = 4; // 16
...
template<int... Is>
constexpr auto calculatePointForSequence(seq<Is...>) -> std::array<Vec,width * height>
{
    return {{ calculatePoint(Is)... }};
}

template<int N>
constexpr auto calculatePoints() -> std::array<Vec,width * height>
{
    return calculatePointForSequence(gen_seq<N-1>{});
}
...
int main(const int /*argc*/, const char */*argv*/[])
{
  ...
  for (int i = 0; i < width * height; i++)
    fprintf(f, "%d %d %d ",
      toInt(calculatePoints<width * height>()[i].x),
      toInt(calculatePoints<width * height>()[i].y),
      toInt(calculatePoints<width * height>()[i].y));
}
